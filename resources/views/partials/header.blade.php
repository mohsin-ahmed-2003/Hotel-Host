<style>
    ...
    .header-dropdown-name  { font-size: 15px; font-weight: 700; color: #ffffff; }
    .header-dropdown-email { font-size: 13px; color: #93afd4; margin-top: 2px; }
    ...
    .header-dropdown a:hover { background: rgba(0,0,0,0.05); }
    body.dark-mode .header-dropdown a:hover { background: rgba(255,255,255,0.05); }
    .header-dropdown a.danger { color: #dc2626; }
    body.dark-mode .header-dropdown a.danger { color: #f87171; }
    .header-dropdown a.danger:hover { background: rgba(220,38,38,0.06); }
    .header-dropdown-divider { height: 1px; background: var(--border); margin: 4px 0; }

    @media (max-width: 1150px) {
        .header-content { padding: 0 12px; gap: 8px; }
        .site-name-text { font-size: 16px; }
        .header-search input { width: 100%; padding: 8px 12px 8px 28px; font-size: 12px; background-position: 8px center; }
    }

    @media (max-width: 768px) {
        .header-container { position: sticky; }
        .header-search { display: none; }
        .header-site-logo-img { height: 36px !important; max-width: 120px !important; }
        .site-name-text { font-size: 16px; }
    ...
    <div class="header-content">
        <a href="/" class="site-logo">
            @if($siteSettings->get('site_logo'))
                <img src="{{ \Illuminate\Support\Facades\Storage::url($siteSettings->get('site_logo')) }}" alt="{{ $siteSettings->get('site_name', 'Hotel Host') }}" class="header-site-logo-img" style="height:48px;width:auto;max-width:160px;object-fit:contain;">
            @else
        ...
        <nav class="homenavbar">
            ...
            <button class="theme-toggle-btn" id="themeToggle" onclick="toggleTheme()" title="Toggle theme">
                <svg id="themeIconDark" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="M21 12.79A9 9 0 1 1 11.21 3 7 7 0 0 0 21 12.79z"/></svg>
            ...
            @if(session('user_id'))
                <div class="user-menu-pill" id="headerProfileWrap" onclick="toggleHeaderDropdown()">
                    ...
                    <img src="{{ asset(session('user')->profile_image ?? 'images/image.png') }}" alt="Profile" class="pill-profile-img">
