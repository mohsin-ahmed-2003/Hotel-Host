...
@section('content')
    <div class="trips-dashboard-wrapper">
        <div class="dashboard-container">
            <!-- Header Section -->
            <div class="dashboard-header">
                <div class="header-left">
                    <h1 class="dashboard-title">My Trips</h1>
                    <p class="dashboard-subtitle">Manage your past and upcoming reservations.</p>
                </div>
                <div class="header-right">
                    ...
                    <div class="header-tabs">
                        <a href="{{ route('user.trips', ['tab' => 'previous']) }}"
                            class="tab-link {{ $tab === 'previous' ? 'active' : '' }}">Previous</a>
                        <a href="{{ route('user.trips', ['tab' => 'current']) }}"
                            class="tab-link {{ $tab === 'current' ? 'active' : '' }}">Current</a>
                        <a href="{{ route('user.trips', ['tab' => 'upcoming']) }}"
            ...
            <div class="reservation-grid" id="reservationGrid">
                    ...
                    <div class="res-card">
                        <div class="res-card-img-wrap">
                            ...
                            <div class="host-profile-overlay">
                                <img src="{{ asset($res->room->user->profile_image ?? 'images/image.png') }}" class="host-img"
                                    alt="Host">
                                <div class="host-overlay-text">
                                    <span class="host-label">Hosted by</span>
                                    <span class="host-name">{{ $res->room->user->name ?? 'Unknown' }}</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="res-card-body">
                            ...
                            <div class="res-actions">
                                <a href="mailto:{{ $res->room->user->email ?? '' }}?subject=Reservation Inquiry #{{ $res->id }}"
                                    class="btn-action btn-message">Message Host</a>
                ...
                @empty
                    <div class="empty-state">
                        ...
                        <a href="/" class="btn-explore" style="margin-top: 20px;">Start searching</a>
                    </div>
                @endforelse
            </div>
    ...
    <script>
        function setViewMode(mode) {
            ...
            if (mode === 'list') {
                ...
                localStorage.setItem('trips_view_mode', 'list');
            } else {
                grid.classList.remove('list-view');
                btns[0].classList.add('active');
                localStorage.setItem('trips_view_mode', 'card');
            }
        }

        // Load saved preference or force list on mobile
        document.addEventListener('DOMContentLoaded', () => {
            const savedMode = localStorage.getItem('trips_view_mode');
