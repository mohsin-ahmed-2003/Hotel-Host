    ...
    <div class="card">
        <div class="card-header">
            <div style="display:flex;align-items:center;gap:12px;">
                <div class="user-avatar" style="width:44px;height:44px;font-size:16px;">{{ strtoupper(substr($user->name,0,1)) }}</div>
                <div>
        ...
        <div class="card-body">
            <form id="editUserForm" action="{{ route('admin.users.update', $user) }}" method="POST" novalidate>
                @csrf @method('PUT')
                <div class="form-grid">
                    <div class="form-group">
                        <label class="form-label">Full Name</label>
                        <input type="text" id="f_name" name="name" class="form-control" value="{{ old('name', $user->name) }}" placeholder="John Doe" required>
                        <div class="field-err" id="e_name"></div>
                    </div>
                    <div class="form-group">
                        <label class="form-label">Email Address</label>
                        <input type="email" id="f_email" name="email" class="form-control" value="{{ old('email', $user->email) }}" placeholder="john@example.com" required>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" id="f_phone" name="phone" class="form-control" value="{{ old('phone', $user->phone) }}" placeholder="+1 555 000 0000" required>
                    ...
                    <div class="form-group">
                        ...
                        <input type="password" id="f_password" name="password" class="form-control" placeholder="Leave blank to keep current">
                        <div class="field-err" id="e_password"></div>
                    </div>
                    <div class="form-group">
                        <label class="form-label">Date of Birth</label>
                        <input type="date" id="f_dob" name="date_of_birth" class="form-control" value="{{ old('date_of_birth', $user->date_of_birth?->format('Y-m-d')) }}" required>
...
<script>
    ...
    document.getElementById('editUserForm').addEventListener('submit', function(e) {
        ...
        if (pw && !pwReg.test(pw)) { showErr('password', 'Min 8 chars with uppercase, lowercase, number & symbol.'); valid = false; }
