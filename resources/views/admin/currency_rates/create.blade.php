...
@section('title', 'Add Currency Rate')
@section('page-title', 'Add Currency Rate')

@section('content')
<div class="card" style="max-width: 600px; margin: 0 auto;">
    <div class="card-header">
        <span class="card-title">Add New Rate (Base: USD)</span>
        <a href="{{ route('admin.currency-rates.index') }}" class="btn btn-outline btn-sm"><i class="fas fa-arrow-left"></i> Back</a>
    </div>
    <div class="card-body">
        <form action="{{ route('admin.currency-rates.store') }}" method="POST">
            @csrf
            <div style="margin-bottom: 16px;">
                ...
                <select name="target_currency" required style="width:100%; padding:10px; border:1px solid var(--border); border-radius:8px; background:var(--bg); color:var(--text);">
                    ...
                    @foreach($currencies as $currency)
                        <option value="{{ $currency->currency_code }}" {{ old('target_currency') == $currency->currency_code ? 'selected' : '' }}>
            ...
            <div style="margin-bottom: 16px;">
                ...
                <input type="number" step="0.00000001" name="rate" value="{{ old('rate') }}" required style="width:100%; padding:10px; border:1px solid var(--border); border-radius:8px; background:var(--bg); color:var(--text);">
            ...
            <div style="margin-bottom: 24px;">
                ...
                <input type="date" name="rate_date" value="{{ old('rate_date', now()->format('Y-m-d')) }}" required style="width:100%; padding:10px; border:1px solid var(--border); border-radius:8px; background:var(--bg); color:var(--text);">
                @error('rate_date') <span style="color:#ef4444; font-size:12px;">{{ $message }}</span> @enderror
            </div>

            <button type="submit" class="btn btn-primary" style="width:100%; justify-content:center; padding:12px; font-size:15px;">Save Rate</button>
        </form>
    </div>
</div>
@endsection
